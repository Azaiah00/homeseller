...
const SmartSellTimeline = () => {
  ...
  const exportToPDF = () => {
    ...
    // Header
    pdf.setFontSize(20)
    pdf.setTextColor(15, 23, 42) // navy color
    pdf.text('Smart-Sell Reverse Timeline', pageWidth / 2, yPosition, { align: 'center' })
    yPosition += 10

    pdf.setFontSize(12)
    pdf.setTextColor(100, 100, 100)
    pdf.text(`Target Closing Date: ${formatDate(new Date(targetDate))}`, pageWidth / 2, yPosition, { align: 'center' })
    yPosition += 15
    ...
    pdf.text('Generated by Frederick Sales - Your personalized roadmap from "Thinking about it" to "Sold."', pageWidth / 2, yPosition, { align: 'center' })
